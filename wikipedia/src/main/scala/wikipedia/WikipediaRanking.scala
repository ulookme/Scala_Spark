...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val conf = new SparkConf()
    ...
    .setAppName("Wikipedia Language Ranking")

  val sc = new SparkContext(conf)
  // Hint: use a combination of `sc.parallelize`, `WikipediaData.lines` and `WikipediaData.parse`
  val wikiRdd = sc.parallelize(WikipediaData.lines.map(WikipediaData.parse))
  ...
  rdd.flatMap(article => langs.filter(article.mentionsLanguage).map(lang => (lang, article)))
     .groupByKey()
  }
